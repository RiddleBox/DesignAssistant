...
✅ 应抽取为 technical 信号（以下任一条件成立）：
  ...
  - 行业报告/分析中指出的技术栈转移或能力门槛变化（有具体厂商/工具/数据支撑）
...
❌ 不应抽取为 technical 信号（以下特征存在时克制）：
  ...
  - 未公开发布的内部研发进展（无原文依据）
  - 泛泛的 AI/技术趋势展望，无具体落地事实或产品发布（"AI 将改变游戏开发"→ 不抽）
...
capital 信号的本质问题是：这个资本事件是否代表有迹可查的资金/所有权/市场结构变化？

✅ 应抽取为 capital 信号（以下任一条件成立）：
  - 已完成的融资、收购、投资（有具体金额或已官方确认）
  - 公司破产/清算/债务重组的正式启动
  - 重大财务减记或资产剥离（已公告）
  - 行业报告中指出的 M&A 周期性趋势或融资规模变化（有具体交易数量/金额/时间区间支撑）

❌ 不应抽取为 capital 信号（以下特征存在时克制）：
  - 意向性表述（"正在考虑融资"、"可能被收购"）— 可抽取但 confidence ≤ 4
  - 资本事件的战略背景解释（"收购是为了进入移动市场"→ 背景说明，不是独立信号）
  - 未经官方确认的传言（confidence ≤ 3）
  - 行业报告中的泛泛预测，无具体交易数据支撑（"预计 M&A 将增加"→ 不抽）
...
# ── 粗筛 Prompt（轻量版，给 haiku 用）──────────────────────────────────
SCREEN_SYSTEM_PROMPT = """你是游戏行业情报粗筛器。你的唯一任务是判断一段文本中是否含有范式信号。

范式信号是指：可追踪的、影响行业格局的具体变化事实，包括技术路线决策、市场结构变化、人员架构变动、资本事件、监管政策变化。

以下内容不是范式信号，直接返回 has_signal=false：
- 泛趋势展望或行业预测（"AI 将改变游戏开发"）
- 无具体事实支撑的意见或分析文章
- 纯经验分享或 GDC 演讲内容
- 续报（对已知事件的简单跟进，无新增事实）

请严格输出如下 JSON，不要任何额外说明：
{"has_signal": true或false, "signal_types": ["technical"|"market"|"team"|"capital"|"regulatory"的子集，无信号时为空数组]}"""


def build_screen_prompt(content: str) -> str:
    """构建粗筛 Prompt（极轻量，不含 few-shot，给 haiku 快速判断）"""
    # 只取前 800 字符，节省 token
    truncated = content[:800]
    return f"""{SCREEN_SYSTEM_PROMPT}
