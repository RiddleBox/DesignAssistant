...
**当前状态**：✅ MVP 完成 | ✅ 验收通过 | ✅ P1-2/P1-3/P1-4 联调完成 | ✅ LLM 三路辩论已上线（鹰/鸽/仲裁，2026-03-27）| ✅ 联调回归通过（2026-03-28，多机会列表输入+新字段对齐）| 📋 后续规划已同步至执行进展

**遗留待办**：

| # | 项目 | 优先级 | 说明 |
|---|------|--------|------|
| 1 | **LLM 路径完整验证** | P1 | `_llm_design` prompt 是否真正体现 first principle（行动设计层/资源承诺层）？需真实案例验证 |
| 2 | **`why_now` 消费** | P1 | 2.2 新增字段，应影响 posture 和 phased_plan 节奏；当前 prompt 是否已利用？ |
| 3 | **`next_validation_questions` 消费** | P1 | 语义已修正为"2.3 前置问题"；需映射到 Go/No-Go 和 key_assumptions_to_test |
| 4 | **验证案例更新** | P1 | 补充游戏行业场景、多机会列表输入 |
| 5 | 细粒度资源模型 / 长篇建议模板 / 多 Agent 编排 / 2.4 深度集成 | P2 | 触发条件满足时启动 |
