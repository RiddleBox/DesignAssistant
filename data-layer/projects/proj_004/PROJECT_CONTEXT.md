...
> **文档类型**：项目状态总览 + 开工上下文
> **最后更新**：2026-04-10（同步 `Phase 3` 执行进度/设计方案、`2.2 opportunity_key` 身份稳定化、报告命名与稳定性观测升级）
...
> `2.1 / 2.2 / 2.3 / 2.4 / 2.5` 的 MVP 与主链路联调均已完成，项目已正式进入 **Phase 3 真实数据管道 + 螺旋质量迭代**。当前最值得关注的是：`2.2` 机会身份稳定性在真实批跑中的持续验证、`2.1` 统一配置后的稳定性治理，以及 `Iteration 5` 所需的外部未见样本质量基线重建。
...
| P0-3 | `Phase 3 Iteration 5` 外部未见样本扩充与质量基线重建 | 2.1 / 2.2 / 2.5 | ⏳ 待规划 |
...
| # | 事项 | 说明 |
|---|------|------|
| 1 | 机会身份稳定性验证 | 用稳定性批跑确认 `opportunity_key_stable`，区分标题漂移与身份漂移 |
| 2 | Iteration 5 质量基线重建 | 用外部未见样本重测 Precision / Recall / 噪音抑制率 |
| 3 | API / provider 稳定性治理 | 补 timeout / provider 对照 / 异常日志，减少随机 fallback |
| 4 | 2.5 跨轮次趋势复盘 | 积累更多真实运行记录，为系统性问题识别建立基础 |
