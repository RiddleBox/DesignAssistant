...
| **模块编号** | `Phase 2.1` |
| **启动日期** | 2026-03-14 |
| **角色协作模式** | 同一 Agent 下的角色面具协作小队（7 个职责视角，执行时可压缩为 5-6 个角色面具） |
| **模块负责人** | 方案设计负责人视角（已完成多角色讨论并产出设计方案） |
| **协作角色视角** | 总协调视角 / 方案设计视角 / 信息抽取视角 / 结构化契约视角 / Prompt策略视角 / 评测验收视角 / 实现落地视角 |
| **角色定义文档** | [phase2.1_roles.md](phase2_roles/phase2.1_roles.md) |
| **上游输入** | `phase2.1_目标说明.md`、`phase2.4_启动与拍板.md`、`phase2.4_进展与待拍板事项.md`、`阶段2团队构建方案.md` |
| **下游服务对象** | `Phase 2.2` 机会评估模块、`Phase 2.5` 整合验证与复盘 |
| **当前状态** | `MVP 实现已完成，待 benchmark 验收` |
| **实现目录** | [phase2.1_implementation/](../proj_004/phase2.1_implementation/) |
...
`2.1` 启动应遵循”**先冻结治理，再定义角色面具，再由多角色收敛设计，最后进入实现**”的顺序，而不是一上来直接写代码，或为了形式完整而机械重跑整套流程。

**重要说明**：
- Phase 2.1 采用**同一 Agent 下的角色面具协作模式**
- 7 个职责视角由同一个 Agent 切换角色面具完成，不是 7 个独立 Agent
- 角色定义落档是指将角色定义写入 `phase2.1_roles.md`，不是创建独立 Agent 档案

#### A. 建队与启动主路径

```text
Skill 0 协调本轮目标
→ Skill 8 分析 2.1 角色缺口与推荐配置
→ 角色定义落档到 phase2.1_roles.md（7 个职责视角，执行时可压缩为 5-6 个角色面具）
→ 方案设计负责人视角牵头组织多角色讨论
...
|------|----------|-------------|--------|----------------|
| **1. 冻结治理入口** | 确认 `2.1` 边界、MVP 范围、依赖模式与待拍板项 | 总协调视角 / 用户 | 本文档首轮确认版 | `7.1` 关键拍板项已拉齐 |
| **2. 定义角色面具** | 明确 7 个职责视角、执行压缩方案、角色协作流程 | 总协调视角 | `phase2.1_roles.md` | 角色定义已落档，职责边界已明确 |
| **3. 多角色收敛设计** | 组织抽取、Schema、Prompt、评测四视角讨论 | 方案设计视角 | `phase2.1_设计方案.md` 草案 | 设计取舍、非目标与主流程已收敛 |
| **4. 完成首轮拍板** | 对关键边界、输出契约、实现策略和验收口径做用户确认 | 总协调视角 / 用户 | 拍板结论回写 | 必拍板项已确认，禁止带着开放分歧进入实现 |
| **5. 启动 MVP 闭环** | 落地 `Prompt-first` 实现、轻量校验、首轮 benchmark 与样例联调 | 实现落地视角 / 评测验收视角 | 可运行闭环、样例结果、基线数据 | Schema 合法率、准确率、召回率达到当前阶段目标 |
| **6. 进入下游联调** | 核对 `2.1 -> 2.2` 消费方式，并复查 `2.4` 增强依赖是否可接 | 结构化契约视角 / 总协调视角 | 联调结果、依赖复查记录 | 下游可稳定消费，上游依赖不构成强阻塞 |

#### C. 执行纪律

- **先定义角色、先讨论、先拍板，再进入实现**：`phase2.1_设计方案.md` 不应凭空先写，而应作为角色定义落档后、多角色讨论的正式产物。
...
- **角色面具协作，不是多 Agent 自治**：7 个职责视角由同一个 Agent 切换角色面具完成，不是 7 个独立 Agent 并行自治。
...
|--------|----------|----------|------------------|----------|
| **模块边界** | A. 只做结构化信号抽取；B. 顺带做机会判断；C. 直接输出建议 | **A** | 会直接决定 `2.1` 是否越界侵入 `2.2 / 2.3` | ✅ **A**（2026-03-14） |
| **MVP实现策略** | A. `Prompt-first + 轻量校验`；B. 同步做复杂规则引擎；C. 多模型投票起步 | **A** | 决定首轮是先打通闭环还是过早追求完整版 | ✅ **A**（2026-03-14） |
| **`2.1 -> 2.2` 输出形态** | A. 自由文本；B. 半结构化章节；C. 固定 JSON + 可选摘要 | **C** | 下游评估必须稳定消费结构化字段 | ✅ **C**（2026-03-14） |
| **首版信号分类体系** | A. 维持 `technical / market / team / capital`；B. 立即扩到 `6-8` 类；C. 暂不分类 | **A** | 决定标注、Prompt 与验收口径是否稳定 | ✅ **A**（2026-03-14） |
| **对 `2.4` 的依赖模式** | A. 低耦合先启动，增强项后接入；B. 等 `2.4` 完全稳定再开始；C. 强依赖当前接口立刻深耦合 | **A** | 决定本周是否能启动 `2.1` 而不被阻塞 | ✅ **A**（2026-03-14） |

### 7.2 本周最好拍板

| 决策项 | 可选方案 | 推荐方案 | 延后风险 | 拍板结果 |
|--------|----------|----------|----------|----------|
| **首轮 benchmark 样本量** | A. `10` 条；B. `30` 条；C. `50+` 条 | **B** | 样本太少无代表性，太多会拖慢启动 | ✅ **B**（2026-03-14） |
| **评分口径定义方式** | A. 仅 Prompt 描述；B. Prompt + 明文评分规则表；C. 先不定义 | **B** | 不提前定义，后面准确率与一致性难比较 | ✅ **B**（2026-03-14） |
| **标注与验收负责人** | A. 测试角色单独负责；B. 用户人工主导；C. 评测负责人主导 + 用户终审 | **C** | 没有冻结责任人，benchmark 难形成正式结论 | ✅ **C**（2026-03-14） |
| **是否首轮引入后处理规则** | A. 先无规则；B. 只加轻量规范化；C. 直接上复杂规则引擎 | **B** | 纯 Prompt 容易漂移，复杂规则又会过重 | ✅ **B**（2026-03-14） |
