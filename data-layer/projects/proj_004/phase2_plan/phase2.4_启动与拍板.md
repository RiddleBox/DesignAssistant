...
|--------|----------|----------|------------------|----------|
| **MVP知识来源策略** | A. 全部手工真实来源整理；B. 真实来源摘要 + 少量样例补齐；C. 先大量AI合成占位 | **B** | 会直接影响 `100` 条数据的质量、进度和可追溯性 | 待定 |
| **Week 1 检索方案边界** | A. 纯向量检索 + 简单生成；B. 直接做混合检索；C. 先只做检索不做生成 | **A** | 决定 Week 1 是否以打通闭环优先，还是提前追求完整版能力 | 待定 |
| **Document最小标准字段** | A. 维持当前最小字段；B. 立即增加更多结构字段；C. 允许字段自由变化 | **A** | 会影响接口稳定性、数据清洗工作量和下游联调难度 | 待定 |
| **MVP对下游的正式交付接口** | A. 冻结 `retrieve / generate / rag`；B. 仅对外暴露 `rag`；C. 继续边做边改不冻结 | **A** | 下游要不要现在开始依赖、以及接口文档是否可以正式编写，取决于此 | 待定 |
