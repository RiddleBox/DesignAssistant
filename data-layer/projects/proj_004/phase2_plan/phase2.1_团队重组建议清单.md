...
**重要**：Phase 2.1 采用**同一 Agent 下的角色面具协作模式**，而非多个独立 Agent 并行自治模式。
...
**为什么不是多 Agent**：
- 多 Agent 模式适用于大规模并行、独立子任务
- Phase 2.1 需要的是多视角讨论的设计收敛，不是独立自治
- 角色面具模式上下文天然共享，决策内部整合，更适合设计阶段

**详细角色定义**：
- 完整的角色定义、职责边界、协作流程见 [phase2.1_roles.md](phase2_roles/phase2.1_roles.md)

### 5.1 推荐编制

建议 `2.1` 采用以下 **7 个职责视角**（执行时可压缩为 5-6 个角色面具）：
