...
### 6.3 优先级 P2：LLM 调用稳定性治理

**目标**：让统一 `llm_config` 之后的真实 provider 调用具备可诊断、可超时、可切换能力

**具体动作**：
1. 给统一 LLM 调用链补齐 timeout 配置
2. 在 decoder / client 层输出更清晰的 provider、model、超时、异常类型日志
3. 用备用 provider 做一次对照验证，区分环境问题与实现问题
4. 稳定后再决定是否收敛当前调试输出

### 6.4 优先级 P3：扩大 Benchmark 样本
