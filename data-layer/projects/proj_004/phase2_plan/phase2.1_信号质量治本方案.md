...
### 十五、`Step 4` 当前可先做的边界预埋（2026-04-02 补充）

在 `Step 3` 与候选集机制尚未完全稳定前，`Step 4` 不建议直接进入完整实现；但为了避免后续倒查接口，可以先把以下三类边界定义清楚。

#### 15.1 触发条件（何时才进入 `Step 4`）

建议只有在以下任一情况出现时，才启用大批次假说驱动 / 漏斗增强：

- 单批次高密度信号明显升高（如 `50+` 条）
- `Step A / Step B` 返回的候选数量明显膨胀，导致 `Step C` 试跑成本过高
- 已出现“候选很多，但有效机会产出率持续偏低”的稳定现象

#### 15.2 输入契约（`Step 4` 未来应消费什么）

`Step 4` 不应重新回到“读自由文本做全量猜测”，而应尽量消费已经结构化的中间层对象：

- 来自 `2.1` 的结构化逻辑字段（如 `logic_frame` / 角色 / domain / 关键约束）
- 来自 `Step A` 的候选场景或软约束结果
- 来自 `Step B` 的来源类型、route、排序分、补槽信息、沉睡激活标记
- 必要时再叠加轻量假说模板，而不是重新覆盖前序规则判断

#### 15.3 观测目标（启动后要优化什么）

`Step 4` 的观测重点建议不是“是否更复杂”，而是是否在大批次下真正带来：

- 候选压缩率提升
- Top-K 候选保真度提升
- `Step C` 尝试次数下降但有效机会产出率不恶化
- 假说覆盖率提升（尤其是跨域、长链、旧场景再激活）

因此，`Step 4` 现阶段最合适的动作是：**先定义触发条件、输入契约、观测指标，不急于实现完整机制。**
