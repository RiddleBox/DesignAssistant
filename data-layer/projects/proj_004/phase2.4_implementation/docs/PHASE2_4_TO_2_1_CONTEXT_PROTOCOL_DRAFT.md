...
> ⚠️ **[SUPERSEDED]** 本文档为 2026-03-14 草案，已被 2026-03-28 拍板结果覆盖。
> **不应再引用本文档中的字段名（尤其是 `use_as`）作为现行设计依据。**
> 现行协议见：`PHASE2_4_CONTEXT_PACKET_PROTOCOL.md`（v1.0，2026-03-28 冻结）
> 关键变更：`use_as` → `content_type`（描述内容性质，不预判用途，由下游模块自行决定如何消费）
